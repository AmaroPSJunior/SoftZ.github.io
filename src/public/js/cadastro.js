...
const Cadastro = (function () {
    ...
    function acoesFormulario() {
        ...
        $("#foto-perfil").on("change", function () {
            let 
                acceptExtension = $(this).attr("accept"),
                extensao = $(this).val().substr(($(this).val().lastIndexOf('.') + 1))
            ;
            acceptExtension = acceptExtension.split(',')
            extensao = "." + extensao
            
            if ($.inArray(extensao, acceptExtension) == -1) {
                $("#foto-perfil").val('./src/public/img/user-default.png')
    ...
    function enviarDados() {
        ...
        $("#salvar-dados").on("click", function () {
            ...
            if (fotoBase64) {
                ...
                let 
                    fotoBase64split = fotoBase64.split('base64,'),
                    base64type = fotoBase64split[0]
                ;
    ...
    function loadDate() {
        ...
        let 
            ...
            linkImg = '/img/user-default.png'
        ;
        
        if (type.length > 0 ) {

            axios.post('http://localhost:3001/auth/authenticate', { email, password })
            .then(function (response) {
